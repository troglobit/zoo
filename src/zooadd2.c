...
/****************
 * This function is called with zoo_file positioned to the first directory
 * entry in an archive.  It skips past all existing files, counts the number
 * of deleted files, saves the latest data and time encountered, and adds
 * all filenames encountered to a global list. The long filename is added if
 * available, else the MSDOS filename is added.
*/
void skip_files(zoo_file, latest_date, latest_time, delcount, latest_name, latest_pos)
ZOOFILE zoo_file;
	...
	*latest_pos = 0L;
	do {
		...
		*latest_pos = save_offset;
		...
		if (!drp->deleted) {
			if (drp->date > *latest_date || (drp->date == *latest_date && drp->time > *latest_time)) {
		...
		if (!drp->deleted) {
			...
			addfname(latest_name, save_offset, drp->date, drp->time, drp->vflag, drp->version_no);
		}
...
/****************
 * deletes all files in the supplied list of pointers to filenames
 */
...
void opts_add(option, zootime, quiet, suppress, move, new, pack,
	      update, add_comment, z_fmt, need_dir, inargs, genson, use_lzh, arch_cmnt)
char *option;
...

{
	...
	while (*option) {
		...
		case 'h':
			(*use_lzh)++;
			break;		 /* use lzh compression */
		case 'f':
			(*suppress)++;
			break;		 /* suppress compression */
		case 'M':
			(*move)++;
			break;		 /* delete files after adding them */
		case 'n':
			(*new)++;
			break;		 /* add only files not in archive */
		case 'P':
			(*pack)++;
			break;		 /* pack after adding */
		case 'u':
			(*update)++;
			break;		 /* add only files already in archive */
		case 'q':
			(*quiet)++;
			break;		 /* be quiet */
		case 'c':
		...
		case ':':
			*need_dir = 0;
			break;		 /* don't store directories */
		case 'I':
			(*inargs)++;
			break;		 /* get filenames from stdin */
		case 'C':
			(*arch_cmnt)++;
			break;		 /* do an archive comment */
/* #ifdef PORTABLE *//* avoid Turbo C warning about unused param */
		case 'z':
			(*z_fmt)++;
			break;		 /* look for Z format files */
/* #endif */
		case '+':
			*genson = 1;
			break;
...
/****************
 * Function getsdtin() gets a pathname from standard input, cleans it if
 * necessary by removing any following blanks/tabs and other junk, and
 * returns it in a static area that is overwritten by each call.
*/
char *getstdin()
{
	...
	do {
		...
		for (chptr = tempname; *chptr != '\0'; chptr++) {
			if (
				/*
				 * PURIFY means remove trailing blanks/tabs
				 * and all subsequent chars
				 */
#ifdef PURIFY
				*chptr == '\t' || *chptr == ' ' ||
#endif
