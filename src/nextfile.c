...
#ifndef SPECNEXT
/**********
 * If portable version, nextfile() is effectively a no-op and any
 * wildcard expansion must be done by the runtime system before the
 * command line is received by this program
 */
char *nextfile(what, filespec, fileset)
int what;		/* whether to initialize or match      */
char *filespec;         /* filespec to match if initializing   */
int fileset;            /* which set of files                  */
{
...
int fileset;		       /* which set of files                  */
{
	...
	if (ffretval != 0) {				/* if error status		*/
		if (first_time[fileset]) {		/*   if file never matched then	*/
			first_time[fileset] = 0;
			return saved_fspec[fileset];	/*      return orig filespec    */
		}
...
/**********
 * fcbpath() accepts a pointer to an ffblk structure, a character
 * pointer *to a pathname that may contain wildcards, and a character
 * pointer to a buffer.  Copies into buffer the path prefix from the
 * pathname and the filename prefix from the ffblk so that it forms a
 * complete path.
 */
...
char *new_path;
{
	...
	/*
	 * either we found a "/", "\", or ":", or we reached the
	 * beginning of the name.  In any case, i points to the last
	 * character of the path part.
	 */
	start_pos = i + 1;
	for (i = 0; i < 13; i++)
