...
char *zoo_path, *option;
{
	...
	static char partial_msg[] = "Partially packed archive left in %s.\n";
	char *temp;
	...
	if (ver_too_high(&old_zoo_header))
		prterror('f', wrong_version, old_zoo_header.major_ver, old_zoo_header.minor_ver);
	...
	if (old_zoo_header.type > 0) {
		...
		/* keep generations off if using old format archive */
	...
	if (old_zoo_header.acmt_len != 0) {
		zooseek(zoo_file, old_zoo_header.acmt_pos, 0);
		getfile(zoo_file, new_file, (long)zoo_header.acmt_len, 0);
	}
	...
	while (1) {
		...
		if (direntry.zoo_tag != ZOO_TAG) {
			...
			if (bad_header) {
				/* bad headers means don't save temp file */
				zooclose(new_file);
				unlink(temp_file);
			} else {
		...
		if (!direntry.deleted) {
			...
			/* nothing */
#else
			prterror('m', "%-14s -- ", direntry.namlen > 0 ? direntry.lfname : direntry.fname);
#endif
			...
			if (zooseek(zoo_file, direntry.offset, 0) == -1L) {
				...
				zooseek(zoo_file, direntry.offset, 0);
				...
				if (status == 0 && direntry.cmt_size != 0) {
					...
					status = getfile(zoo_file, new_file, (long)direntry.cmt_size, 0);
				}
				...
				if (status != 0) {
					...
					if (latest_date < direntry.date ||
					    (latest_date == direntry.date && latest_time < direntry.time)) {
