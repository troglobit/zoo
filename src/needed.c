...
struct zoo_header *header;
{
	...
	if (arg_count <= FIRST_ARG &&
	    (!(header->vdata   & VFL_ON) ||
	     !(direntry->vflag & VFL_ON) ||
	      (direntry->vflag & VFL_LAST)))
	...
	for (i = arg_count - 1; i >= next_arg; i--) {
		...
		/*
		 * If the argument contains a slash, the match fails if
		 * the path prefixes don't match
		 */
		...
		if (p != NULL && p != lastptr(arg_copy)) {	/* nonnull version in arg */
			if (q != NULL) {		/* nonnull ver. in archive */
				...
				if (STRCMP(pp, !=, "0") && STRCMP(pp, !=, "^0") && !match_half(qq, pp)) {
					debug(("needed:  no match versions [%s] and [%s]\n", qq, pp))
 ...
 * the pattern are: "*" for zero or more arbitrary characters; "?"  for
 * any one characters.  Unlike the MS-DOS wildcard match, "*" is
 * correctly handled even if it isn't at the end of the pattern. ".'  is
 * not special.
 *
 * Originally written by Jeff Damens of Columbia University Center for
 * Computing Activities.  Taken from the source code for C-Kermit
 * version 4C.
 */
